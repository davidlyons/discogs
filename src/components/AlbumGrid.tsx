...
export const AlbumGrid = ({ releases }: { releases: ReleaseFormatted[] }) => (
  <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">
    {releases.map(({ url, title, artists, year, format, thumb }, index) => {
      ...
      const listenUrl = `https://music.youtube.com/search?q=${encodeURIComponent(title)} ${encodeURIComponent(artistString)}`
